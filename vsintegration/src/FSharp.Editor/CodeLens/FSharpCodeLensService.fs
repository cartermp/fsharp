...
type internal FSharpCodeLensService
    ...
    let mutable lastResults = Dictionary<int, ITrackingSpan * CodeLens>()
    ...
    let executeCodeLenseAsync () =  
        asyncMaybe {
            ...
            for symbolUse in symbolUses do
                if symbolUse.IsFromDefinition then
                    ...
                    | :? FSharpMemberOrFunctionOrValue as func when func.IsModuleValueOrMember || func.IsProperty ->
                        let funcID = Hash.combine (hash func.LogicalName) (hash func.FullType)
                        // Use a combination of the the function name + the hashed value of the type signature
                        let typeSignatureHash = hash func.FullType
                        ...
                        if lastResults.ContainsKey funcID then
                            ...
                            if codeLens.TypeSignatureHash = typeSignatureHash then
                            ...
                            unattachedSymbols.Add((symbolUse, func, funcID, typeSignatureHash))
                    | _ -> ()
            ...
            for unattachedSymbol in unattachedSymbols do
                let symbolUse, func, funcID, typeSignatureHash = unattachedSymbol
                let declarationLine, range = 
                    match visit func.DeclarationLocation.Start parsedInput with
                    | Some range -> range.StartLine - 1, range
                    | _ -> func.DeclarationLocation.StartLine - 1, func.DeclarationLocation
                ...
                let test (v: KeyValuePair<_, _>) =
                    ...
                    codeLens.TypeSignatureHash = typeSignatureHash
...
#if DEBUG
                            logWarningf "Couldn't retrieve code lens information for %A" codeLens.TypeSignatureHash
#endif
                            ()
            ...
            for value in tagsToUpdate do
                ...
                codeLensDisplayService.RemoveCodeLens trackingSpan |> ignore
                let Grid = codeLensDisplayService.AddCodeLens newTrackingSpan
                if codeLens.Computed && (isNull codeLens.UiElement |> not) then
                    let uiElement = codeLens.UiElement
                    codeLensDisplayService.AddUiElementToCodeLensOnce (newTrackingSpan, uiElement)
                else
